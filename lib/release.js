...
var exports = module.exports = function(opt, callback) {
  ...
  var packager = _.applyMatches('::packager', fis.media().getSortedMatches());
...
function buildPack(ret) {
  ...
  Object.keys(src).forEach(function(subpath) {
    ...
    pack[packTo].push(file);
  });

  Object.keys(pack).forEach(function(pkg) {
    pack[pkg] = pack[pkg]

    // 根据第一次命中的位置排序。
    .sort(function(a, b) {
        // 根据第一命中的位置排序。
        a = a.__packToFIndex >> 0;
        b = b.__packToFIndex >> 0;
