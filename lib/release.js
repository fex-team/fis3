...
var exports = module.exports = function(opt, callback) {
  ...
  // 通过 env 的配置来决定。
  var config = fis.env();
  opt.md5 = config.get('cli.options.md5', opt.md5)>>0;
  opt.hash = opt.md5 > 0;
  opt.lint = config.get('cli.options.lint', opt.lint);
  opt.test = config.get('cli.options.test', opt.test);
  opt.optimize = config.get('cli.options.optimize', opt.optimize);
  ...
  fis.on('compile:start', function(file) {
    if (!cacheCompiled[file.subpath]) {
      cacheCompiled[file.subpath] = 1;
      compiled.push(file);
    }
