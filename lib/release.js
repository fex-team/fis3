...
var onCompileStart, onCompileEnd, onProcessEnd, onAddFileToCompile;

var useMutliCore = false;
 ...
 * @name release
 */
var exports = module.exports = function(opt, callback, onError) {
  ...
  var q = async.queue(function(file, callback) {
    ...
    useMutliCore && (opt.fromIPC = true);
    fis.compile.async(file, opt, function(error) {
      if (error) {
        returnError(error);
        q.kill();
      } else {
  ...
  q.drain = function(error) {
    ...
    useMutliCore && require('./multi-process.js').killAll();
    // fis.util.write('./profile.json', JSON.stringify(profiles, null, 4));
  ...
  onAddFileToCompile && fis.removeListener('compile:add', onAddFileToCompile);
  fis.on('compile:add', (onAddFileToCompile = function(file) {
    q.push(file);
  }));
