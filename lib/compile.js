...
var exports = module.exports = function(file) {
  ...
  file.links.forEach(function(subpath) {
    exports(fis.project.getProjectPath() + subpath);
  });
  unlock(file);
  return file;
};
...
var map = exports.lang = (function() {
  ...
  ['require', 'jsRequire', 'embed', 'jsEmbed', 'async', 'jsAsync', 'uri', 'dep', 'id', 'xlang'].forEach(map.add);
...
function builtinStandard(content, file, conf) {
  if (typeof content === 'string') {
    fis.log.debug('builtin standard start');
    var type;
    ...
    fis.log.debug('builtin standard end');
  }
  return content;
}
...
function postStandard(file) {
  ...
  if (typeof content === 'string') {
    ...
    while (map.reg.test(content)) {
      content = content.replace(map.reg, function(all, type, depth, value, extra) {
        var ret = '',
          info, id;
        try {
          switch (type) {
            ...
            case 'xlang':
              ...
              if (value.trim()) {
                var partial = file.realpathNoExt + '.partial.' + ext;
                var f = fis.file.wrap(partial);
                f.cache = file.cache;
                f.isPartial = true;
                f.isInline = true;
                f.setContent(value);
