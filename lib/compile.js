...
function xLang(tag, content, file, defaultExt) {
  var m = /\s+type\s*=\s*(['"]?)(?:text|application)\/(.*?)\1/i.exec(tag);
  var ext = defaultExt;

  if (m) {
    var xlang = m[2].toLocaleLowerCase();
    ...
    switch (xlang) {
      ...
      default:
        if (xlang.substring(0, 2) === 'x-') {
          ext = xLang.substring(2);
        }
        break;
    }
  }
  ...
  if (m) {
    ...
    tag = tag.replace(m, function(all, quote) {
