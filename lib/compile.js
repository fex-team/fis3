...
var map = exports.lang = (function() {
  var keywords = ['require', 'embed', 'uri', 'dep', 'jsEmbed', 'async'],
    LD = '<<<',
    RD = '>>>',
...
function process(file) {
  ...
  if (file.rExt) {
    ...
    if (file.optimize || exports.settings.optimize && file.useOptimizer !== false) {
      ...
      try {
        switch (type) {
          ...
          case 'async':
            ...
            ~file.extras.async.indexOf(info.id) || file.extras.async.push(info.id);
            ret = info.quote + info.id + info.quote;
            break;
          case 'uri':
