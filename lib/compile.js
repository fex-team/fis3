...
var exports = module.exports = function(file) {
  ...
  // 避免重复 compile.
  if (~compiled.indexOf(file.subpath)) {
    return file;
  }
  compiled.push(file.subpath);
...
var map = exports.lang = (function() {
  var keywords = ['require', 'embed', 'uri', 'dep', 'jsEmbed', 'async', 'id'],
    LD = '<<<',
    RD = '>>>',
...
function pipe(file, type, ext, keep) {
  var key = type + ext;
  // var processors = fis.config.get('modules.' + key);
  // if (processors) {
  ...
  if (processors.length) {
    ...
    processors.forEach(function(processor, index) {
      ...
      if (typeof processor === 'function') {
        var settings = fis.config.env().get('settings.' + key, {});
        if (processor.defaultOptions) {
          settings = _.merge(processor.defaultOptions, settings);
        }

        if (options) {
          settings = _.merge(options, settings);
        }
