...
var multiProcess = require('./multi-process.js');
...
var hasSetImmediate = typeof global.setImmediate === "function";
function asap(fn) {
  if (hasSetImmediate) {
    setImmediate(fn);
  } else {
    global.process.nextTick(fn);
  }
}
 ...
 * @deprecated 请用 fis.compile.async 代替。
 */
var deprecatedWarned = false;
var exports = module.exports = function(file, context) {
  if (!deprecatedWarned) {
    fis.log.warn('fis.compile is deprecated, please use fis.compile.async instead.');
...
/**
 * 异步编译相关函数入口, 输入为 文件对象，没有输出，直接修改文件对象内容。
 *
 * 默认文件编译会尝试从缓存中读取，如果缓存有效，将跳过编译。
 *
 * @example
 ...
 * @function
 * @param {String} file 文件对象
 * @param {Object} [context] 上下文，比如当时 watch 启动时，会传入 {fromWatch: true}
 * @param {function} callback 回调函数。
 * @namespace fis.compile
 * @deprecated 请用 fis.compile.async 代替。
 */
function compileAsync(file, context, callback) {
  if (!CACHE_DIR) {
    fis.log.error('uninitialized compile cache directory.');
  }
  file = fis.file.wrap(file);
  ...
  if (file.isText() && exports.settings.useLint) {
    tasks.push(function(callback) {
      pipeAsync(file, 'lint', true, callback);
    });
  }
    ...
    if (file.isFile()) {
      if (file.useCompile && file.ext && file.ext !== '.') {
        ...
        if (file.useCache && cache.revert(revertObj)) {
          ...
          if (!context.fromIPC) {
            processAsync(file, context, function(error) {
              if (error) {
            ...
            multiProcess(file, context, function(error) {
              if (error) {
                return callback(error);
              }
      ...
      processAsync(file, context, callback);
    }
  });
 ...
 * @name process
 * @function
 * @deprecated 请使用 fis.compile.processAsync 代替。
 */
...
/**
 * 单文件编译异步入口，与 {@link fis.compile} 不同的是，此方法内部不进行缓存判断。
 * @param  {File} file 文件对象
 * @memberOf fis.compile
 * @name process
 * @function
 */
function processAsync(file, context, callback) {
  fis.log.debug('process:start [' + file.subpath + '] start');
  fis.emit('process:start', file);

  async.series([
    pipeAsync.bind(null, file, 'parser'),
    pipeAsync.bind(null, file, 'preprocessor'),
    pipeAsync.bind(null, file, 'standard'),
    postStandardAsync.bind(null, file, context),
    pipeAsync.bind(null, file, 'postprocessor'),
    pipeAsync.bind(null, file, 'optimizer'),
  ], function(error) {
    fis.log.debug('process:end [' + file.subpath + '] start');
    fis.emit('process:end', file);
    callback(error);
  });
}
 ...
 * @param {Boolean} [keep] 是否保留文件内容。
 */
function pipeAsync(file, type, keep, done) {
  ...
  if (processors.length) {
    ...
    var callback = function(processor, settings, key, next) {
      settings.filename = file.realpath;
      var content = file.getContent();
      ...
      fis.log.debug('pipe [' + key + '] start');
      var onEnd = function(error, result) {
        if (error) {
      ...
      if (processor.length > 3) {
        processor(content, file, settings, onEnd);
      } else {
        try {
          var result = processor(content, file, settings);

          if (typeof result === 'function') { // thunk
            result(onEnd);
          } else if (result && result.then) { // promise
            result.then(onResult).catch(onEnd);
          } else { // sync mode
            onResult(result);
          }
        } catch (e) {
          fis.log.error(`Error: %s\n%s`, e.message, e.stack);
          // onEnd(e || 'error');
        }
      }
    };

    async.forEachOf(processors, function(processor, index, next) {
      var typeOf = typeof processor,
        key, options;
      ...
      if (typeof processor === 'function') {
        ...
        callback(processor, settings, key, next);
      } else {
        fis.log.warning('invalid processor [modules.' + key + ']');
      }
    }, done);
...
function asyncReplace(regex, str, replacer, done) {
  regex.lastIndex = 0;
  ...
  if(match==null) { // No matches, we are done.
    ...
    replacer.apply(null, match.concat(function(err, result) {
      ...
      if (regex.global) { // Keep replacing
        asyncReplace(regex, accum + rest, replacer, function(err, ret) {
          done(err, ret);
        });
      } else {
 ...
 * @param  {file} file 文件对象
 */
function postStandardAsync(file, context, callback) {
  fis.emit('standard:restore:start', file);
  var content = file.getContent();
  ...
  var handlers = {
    id: function(all, type, depth, value, extra, next) {
      ...
      next(null, {ret: ret, info: info});
    },

    moduleId: function(all, type, depth, value, extra, next) {
      var info = fis.project.lookup(value, file);
      var ret = info.quote + info.moduleId + info.quote;
    ...
    hash: function(all, type, depth, value, extra, next) {
      var info = fis.project.lookup(value, file);
      var ret;
      ...
      if (info.file && info.file.isFile()) {
        ...
        if (!locked) {
          lock(file, info.file);
          return exports.async(info.file, context, function(error) {
            if (error) {
              return next(error);
            }
            ...
            var md5 = info.file.getHash();
            var ret = info.quote + md5 + info.quote;
            next(null, {ret: ret, info: info});
          });
        }
        var md5 = info.file.getHash();
    ...
    'require': function(all, type, depth, value, extra, next) {
      var info = fis.project.lookup(value, file);
      file.addRequire(info.id);
    ...
    jsRequire: function(all, type, depth, value, extra, next) {
      var info = fis.project.lookup(value, file);
      file.addRequire(info.id);
    ...
    async: function(all, type, depth, value, extra, next) {
      var info = fis.project.lookup(value, file);
      file.addAsyncRequire(info.id);
    ...
    jsAsync: function(all, type, depth, value, extra, next) {
      var info = fis.project.lookup(value, file);
      file.addAsyncRequire(info.id);
    ...
    uri: function(all, type, depth, value, extra, next) {
      var info = fis.project.lookup(value, file);
      var ret;
    ...
    dep: function(all, type, depth, value, extra, next) {
      var info;
      ...
      next(null, {ret: '', info: info});
    },

    embed: function(all, type, depth, value, extra, next) {
      var info = fis.project.lookup(value, file);
      var f;
      if (info.file) {
        f = info.file;
      ...
      if (f && f.isFile()) {
        ...
        if (!locked) {
          lock(file, f);
          f.isInline = true;
          exports.async(f, context, function() {
            unlock(f);
            addDeps(file, f);
            copyInfo(f, file);
            var ret;
    ...
    xlang: function(all, type, depth, value, extra, next) {
      partialAsync(value, file, extra, function(error, ret) {
        if (error) {
          return next(error);
        }
    ...
    inlineStyle: function(all, type, depth, value, extra, next) {
      partialAsync('inline-style-placeholder {' + value + '}', file, extra, function(error, ret) {
        if (error) {
          return next(error);
        }
    ...
    sourceMap: function(all, type, depth, value, extra, next) {
      var info = fis.project.lookup(value, file);
      var ret;
      ...
      if (info.file && info.file.isFile()) {
        ...
        if (info.file.useHash) {
          ...
          if (!locked) {
            ...
            var url = info.file.getUrl();
            var hash = info.hash || info.file.hash;
            var ret = info.quote + url + query + hash + info.quote;
    ...
    info: function(all, type, depth, value, extra, next) {
      next(null, '');
    }

  };

  handlers.jsEmbed = handlers.embed;

  asyncReplace(map.reg, content, function(all, type, depth, value, extra, next) {
    if (!map[type]) {
      fis.log.error('unsupported fis language tag [%s]', type);
    }

    return handlers[type].call(this, all, type, depth, value, extra, function(error, ret) {
      if (!error) {
        var message = typeof ret === 'object' ? ret : { ret: ret};
        ret = message.ret;
    ...
    if (error) {
      lockedMap = {};
      error.message = error.message + ' in [' + file.subpath + ']';
      return callback(error);
    }
 ...
 * // 继续走之后的 js parser 流程。
 */
function partialAsync(content, host, info, done) {
  ...
  if (content.trim()) {
    ...
    f.isInline = true;
    f.setContent(content);
    return processAsync(f, {}, function(error) {
      if (error) {
        return done(error);
      }
