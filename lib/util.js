...
/**
 * 定义exports，处理path，可处理多个参数或者对象或者String传入的路径处理
 * @param  {String} path
 * @return {String}
 */
...
/**
 * 若rPath为文件夹，夹遍历目录下符合查找规则的全部文件；若rPath为文件直接匹配该文件路径是否符合include和exclude规则
 ...
 * @return {Object}
 * @example
 * str = http://a.b.c/f.php?kw=%B2%E5%BB%AD#addfhubqwek
 *                      {
 *                         origin: 'http://a.b.c/f.php?kw=%B2%E5%BB%AD#addfhubqwek',
 *                         rest: 'http://a.b.c/f',
 *                         hash: '#addfhubqwek',
 *                         query: '?kw=%B2%E5%BB%AD',
 *                         fullname: 'http://a.b.c/f.php',
 *                         dirname: 'http://a.b.c',
 *                         ext: '.php',
 *                         filename: 'f',
 *                         basename: 'f.php'
...
/**
 * url处理，提取url中path部分(?之前)、query部分(?#之间)、hash部分(#之后)
...
_.glob = function(pattern, str, options) {
  ...
  if (hasBracket) {
    regex = new RegExp(regex.source.replace(/\uFFF0\\\//g, '(').replace(/\\\/\uFFF1/g, ')'), regex.ignoreCase ? 'i' : '');
  }
