...
_.seralize = function() {
  var datas = [].slice.call(arguments).map(function(item) {
    if (item instanceof Buffer) {
      return item;
    }

    return new Buffer(JSON.stringify(item), "utf8");
  });

  var meta = new Buffer(4 * (datas.length + 2));
  meta.writeUInt32BE(meta.length, 4);
  var size = meta.length;

  for (var i = 0, len = datas.length; i < len; i++) {
    meta.writeUInt32BE(datas[i].length, (i + 2) * 4);
    size += datas[i].length;
  }

  datas.unshift(meta);
  meta.writeUInt32BE(size, 0);
  return Buffer.concat(datas, size);
};

_.unSeralize = function(buf) {
  if (!_.unSeralizeValid(buf)) {
    return null;
  }

  var metaLength = buf.readUInt32BE(4);
  var start = metaLength;
  var ret = [];
  var size, subbuf, str;

  for (var i = 2, len = metaLength / 4; i < len; i++) {
    size = buf.readUInt32BE(i * 4);

    subbuf = buf.slice(start, start + size);
    str = subbuf.toString("utf8");
...
_.unSeralizeValid = function(buf) {
  var totalSize = buf.readUInt32BE(0);
