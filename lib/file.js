...
var File = Object.derive(function() {
  ...
  if (realpath.indexOf(root) === 0) {
    ...
    applyMatches(properties, subpath, fis.media().getSortedMatches());
  }
  ...
  getUrl: function() {
    ...
    if (this.useDomain) {
      ...
      url = path.relative(url, this.relative);
    }
  ...
  addSameNameRequire: function(ext) {
    ...
    } else if ((map = fis.config.env().get('project.ext'))) {
      for (var key in map) {
