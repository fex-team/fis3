...
/**
 * File，fis 编译过程中，文件会被此类进行包装，对于文件的操作，都是通过此类来完成。
 *
...
var File = Object.derive(function() {
   ...
   * 定义默认的文件类型
   *
   * 提取到一个方法里面原因是：当 file 对象从 cache 里面反序列回来后，因为是通过 `Object.defineProperties`
   * 定义的，所以 isXxxLike 系列的属性都会丢失。 所以这块需要重新调用（定义）一次。
   * @function
   * @return {Undefined}
   * @memberOf module:fis.file~File.prototype
   */
 ...
 *
 * @module fis.file
 * @see module:fis.file~File
 */
module.exports = File.factory();

/**
 * 用来包装文件，输入可以路径或者文件对象，输出为文件对象。
 * @param  {Mixed} file 文件路径，或者文件对象
 * @return {File}      返回fis File对象
 * @example
