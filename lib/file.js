...
function getReleaseExt(ext) {
  if (ext) {
    var rExt = fis.config.env().get('roadmap.ext' + ext);
    if (rExt) {
...
function getDomainsByPath(path) {
  var domain = fis.config.env().get('roadmap.domain', {}),
    value = [];
...
var File = Object.derive(function() {
  ...
  if (realpath.indexOf(root) === 0) {
    ...
    applyMatches(this, this.subpath.substring(1), fis.config.env().getSortedMathes());
  ...
  addSameNameRequire: function(ext) {
    ...
    if (fis.util.isFile(this.realpathNoExt + ext)) {
      ...
      var map = fis.config.env().get('roadmap.ext');
      for (var key in map) {
