...
class Bingo_View_Script
{
	...
	protected $_root_path = ''; //模板跟路径
	protected $_module = ''; //当前模块
	
	protected $_templateLoader;	//template加载器
	protected $_widgetLoader;	//widget加载器
	protected $_serviceLoader;	//service加载器
	protected $_componentLoader;	//component加载器
	protected $_layoutLoader;	//layout加载器
	protected $_resource_map; //资源配置表 
	...
	public static function getInstance()
    {
        if (null === self::$_instance) {
			if(constant('USING_BIG_PIPE')){
				require_once('BigPipe/BigPipe.php');
				self::$_instance = Bingo_View_BigPipe::getInstance();
			} else {
