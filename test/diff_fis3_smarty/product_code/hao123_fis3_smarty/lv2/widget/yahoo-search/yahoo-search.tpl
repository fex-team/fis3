...
<%style%>
    @import url('/widget/yahoo-search/ltr/ltr.css?__inline');
<%/style%>
...
App.conf = {
    ...
    , tpl: {
        resultLi: '<li class="r-li"><div class="r-li_hd"><h3><a href="#{REDIRECTURL}">#{TITLE.HTML.HL}</a></h3></div><div class="r-li_tp cf"><span class="r-li_u"><abbr title="#{DISPURL}">#{DISPURL}</abbr></span></div><div class="r-li_bd"><p>#{ABSTRACT.BEST.HTML.HL}</p></div></li>'

        , pnLi: '<li><a href="#{url}" data-n="#{n}" target="_self">#{t}</a></li>'

        , adLi: '<li class="r-li"><div class="r-li_hd"><h3><a href="#{url}">#{t}</a></h3></div><div class="r-li_bd"><p class="r-li_host">#{host}</p><p>#{des}</div><div class="r-li_ft"><div class="r-li_quick">#{quickLink}</div></div></li>'
        ...
        , wordLi: '<li class="r-li_w"><a href="#{url}" target="_self" class="r-li_wa" data-q="#{data}">#{dataFm}</a></li>'
    }
...
require.async("lv2:widget/yahoo-search/yahoo-search-async.js");
<%/script%>
