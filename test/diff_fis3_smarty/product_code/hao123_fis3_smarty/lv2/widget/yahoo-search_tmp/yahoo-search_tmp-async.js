...
App.amazonAds = {
    ...
    callF: null,
    MAXNUM: 2,
    tpl: '<li class="amz-li"><h3><a href="#{href}" data-i="#{i}">#{title}</a>'
        + '<span class="li-img" title="#{brand}"><span>#{brand}</span>'
        + '<img #{imgSrc} #{other}></span></h3>'
        + '<div class="ellipsis"><div><p><span class="dis-pri">#{discount}#{dis}</span>'
        + '<span class="ori-pri">#{price}#{pri}</span>'
        + '<span class="li-des">#{descrip}</span></p></div></div></li>',
    ...
    render: function(data, rand) {
        ...
        if(data.length) {
            index = rand ? _self.getRandsIndex(data.length, max) : _self.enum(max);
...
App.init = function() {
    ...
    G.delegate(document.body, "mousedown", function(e) {
        ...
        (this.tagName === "A" || this.parentNode.tagName === "A" || this.tagName === "BUTTON") && UT && UT.send({type: "click", postion: "links"});
    ...
    G.each(conf.adMaps, function(val, key) {
        G.delegate(val, "mousedown", function(e) {
            (this.tagName === "A" || this.parentNode.tagName === "A") && !/r-list_more/.test(this.parentNode.className) && UT && UT.send({type: "click", position:"search_ad", value: (this.innerText || this.innerHTML).trim(), sort: key, linkIndex: this.getAttribute("data-i") || this.parentNode.getAttribute("data-i")});
        });
    });
    ...
    G.delegate(G("searchLinks"), "mousedown", function(e) {
        UT && UT.send({type: "click", sort: "search_tab", postion: "links", value: this.innerHTML.trim()});
    });
