...
<%style%>
    @import url('/widget/yahoo-search-partner/ltr/ltr.css?__inline');
<%/style%>
...
/*App.conf.reqParam.tn = conf.UT.tn;*/
conf.UT.tn = (conf.UT.tn === "/" ? "" : conf.UT.tn);
conf.UT.params.ytn = conf.UT.tn;
App.conf.tn = conf.UT.tn;
App.conf.reqAdParam.type = conf.UT.tn;
conf.ept = <%json_encode($body.eptWord)%>;
conf.amazonAds = <%json_encode($body.amazonAds)%> || {};

require.async("lv2:widget/yahoo-search-partner/yahoo-search-partner-async.js");
<%/script%>
