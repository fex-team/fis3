...
App.amazonAds = {
    ...
    formatData: function() {
        ...
        if(data) {
            UT && UT.send({modId: "amazonAds_partner", type: "others", position: data.length || 0});
        }
...
App.ept.prototype = {
    ...
    bindCtrl: function() {
        ...
        G.delegate(that.ctrPrev, 'click', function(event) {
            that.getPrev();
            UT && UT.send({modId: "empty-search-rank", type: "pclick", position: "emptyPrev"});
        });
        G.delegate(that.ctrNext, 'click', function(event) {
            that.getNext();
            UT && UT.send({modId: "empty-search-rank", type: "pclick", position: "emptyNext"});
        });
    },
    bindEvent: function() {
        var that = this;
        G.delegate(that.eptTab, 'click', function(event) {
            ...
            if(_self.tagName === "LI" && !_self.className.match("cur")) {
                ...
                UT && UT.send({modId: "empty-search-rank", type: "pclick", position: "emptyTab", sort: that.curType});
            }
        });
        G.delegate(that.eptRes, 'click', function(event) {
            var _self = this;
            if(_self.tagName === "A" || _self.parentNode.tagName === "A") {
                UT && UT.send({modId: "empty-search-rank", type: "pclick", position: "searchWords", sort: that.curType});
            }
        });
        G.delegate(that.eptTle, 'click', function(event) {
            var _self = this;
            if(_self.tagName === "A" || _self.parentNode.tagName === "A") {
                UT && UT.send({modId: "empty-search-rank", type: "pclick", position: "title"});
            }
        });
    },
    createHtml: function() {
...
App.renderPn = function(str) {
    ...
    var conf = App.conf,
        ...
        ret = [],
        buildUrl = function(i) {
            return App.supportState ? "javascript:void(0)" : App.cache.urlbase + "?query=" + G.urlEscape(encodeURIComponent(conf.reqParam.query)) + "&pn=" + i + "&tn=" + conf.tn;
        }
...
App.updateAd = function() {
    ...
    reqAdParam.Keywords && G.ajax({
        ...
        success: function(res) {
            ...
            App.renderAd(data);
            UT && UT.send({type: "others", modId: "yahoo-search-ad_partner", position: conf.tn, sort: "north" + counter.north});
        },
        error: function() {
            App.renderAd(data);
            UT && UT.send({type: "others", modId: "yahoo-search-ad_partner", position: conf.tn, sort: "north" + counter.north});
...
App.renderWord = function(data) {
    ...
    if(wordList) {
        G.each(wordList, function(li) {
            ...
            li.url = App.supportState ? "javascript:void(0)" : App.cache.urlbase + "?query=" + G.urlEscape(encodeURIComponent(li.data)) + "&pn=" + 1 + "&tn=" + conf.tn;
...
App.sugInit = function() {
    ...
    G.loadJs([conf.sugPath], function() {
        ...
        G.delegate(sug.sugWrap, "mousedown", function(e) {
            ...
            (this.tagName === "LI" || this.parentNode.tagName === "LI") && UT && UT.send({type: "pclick", sort:"search_sug"});
...
App.init = function() {
    ...
    G.delegate(document.body, "mousedown", function(e) {
        ...
        (this.tagName === "A" || this.parentNode.tagName === "A" || this.tagName === "BUTTON") && UT && UT.send({type: "pclick", position: "links"});

        (this.id === "searchSubmit" || this.id === "searchSubmit2") && UT && UT.send({type: "pclick", sort: "search_btn"});

        (/logo-hao123/.test(this.className) || /logo-hao123/.test(this.parentNode.className)) && UT && UT.send({type: "pclick", sort:"logo_hao123"});

        (/logo-yahoo/.test(this.className) || /logo-yahoo/.test(this.parentNode.className)) && UT && UT.send({type: "pclick", sort:"logo-yahoo"});
    ...
    G.delegate(G("result"), "mousedown", function(e) {
        (this.tagName === "A" || this.parentNode.tagName === "A") && UT && UT.send({type: "pclick", sort:"search_result", position: "links"});
    });
    ...
    G.each(conf.adMaps, function(val, key) {
        G.delegate(val, "mousedown", function(e) {
            ...
            (this.tagName === "A" || this.parentNode.tagName === "A" || this.parentNode.parentNode.tagName === "A") && !/r-list_more/.test(this.parentNode.className) && UT && UT.send({type: "pclick", position:"search_ad", value: (this.innerText || this.innerHTML).trim(), sort: key, linkIndex: this.getAttribute("data-i") || this.parentNode.getAttribute("data-i") || this.parentNode.parentNode.getAttribute("data-i")});
        });
    });
    ...
    if(App.supportState) {
        history.pushState({
            ...
            , ""
            , App.cache.urlbase + "?query=" + G.urlEscape(encodeURIComponent(conf.reqParam.query)) + "&pn=" + conf.reqPn + "&tn=" + conf.tn
        );
    }
    ...
    G("searchForm").onsubmit = G("searchForm2").onsubmit = function(e) {
        ...
        if(!App.supportState) {
            var tnEle = document.createElement("input");
            tnEle["type"] = "hidden";
            tnEle["name"] = "tn";
            tnEle["value"] = conf.tn;
            this.appendChild(tnEle);
            return;
        }
        conf.reqPn = 1;
        App.req();
        ...
        history.pushState({
            ...
            , ""
            , App.cache.urlbase + "?query=" + G.urlEscape(encodeURIComponent(query)) + "&pn=" + conf.reqPn + "&tn=" + conf.tn
        );
        setTimeout(function() {
    ...
    G.delegate(G("resultWord"), "mousedown", function(e) {
        if(this.tagName === "B"||this.tagName === "A"){
            ...
            UT && UT.send({type: "pclick", modId: "related-keywords", sort: q});
            ...
            history.pushState({
                ...
                , ""
                , App.cache.urlbase + "?query=" + G.urlEscape(encodeURIComponent(q)) + "&pn=" + conf.reqPn + "&tn=" + conf.tn
            );
    ...
    G.delegate(G("searchLinks"), "mousedown", function(e) {
        UT && UT.send({type: "pclick", sort: "search_tab", position: "links", value: this.innerHTML.trim()});
    });
...
/*        setTimeout(function() {
    ...
    G.delegate(G("resultPn"), "mousedown", function(e) {
        ...
        UT && UT.send({type: "pclick", sort:"nextpage"});
