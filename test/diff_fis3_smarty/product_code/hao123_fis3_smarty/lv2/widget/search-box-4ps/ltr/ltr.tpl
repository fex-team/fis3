		...
		<div class="box-search_inner cf">
			...
			<div class="box-search_form">
				...
				<div id="searchGroupRadios" class="radios">
					...
					<%foreach $sBoxTag[0].engine as $value%>
                	<%if empty($body.searchboxEngine[$value.id])%>
					<label for="searchGroupRadio_<%$logoIndex%>"><input id="searchGroupRadio_<%$logoIndex%>" name="searchGroupRadio" value="<%$logoIndex%>" type="radio" <%if $logoIndex == 0%>checked<%/if%>><%$value.title%></label>
