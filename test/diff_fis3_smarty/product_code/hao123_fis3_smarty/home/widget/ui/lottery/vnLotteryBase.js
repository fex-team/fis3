...
VnLotteryBase.prototype._getCityByDate = function(date){
	...
	$.ajax({
		...
		// url: "/static/home/widget/ui/lottery/city.json",
		dataType: "jsonp",
		jsonp: "jsonp",
		jsonpCallback: "ghao123_" + hex_md5(params,16),
		// jsonpCallback: "ghao123_1005831968c4d6b6",
	    cache: false,
...
VnLotteryBase.prototype._getWinningResult = function(date, city, code){
	...
	$.ajax({
		url: self._conf.apiUrlPrefix + params,
		// url: "/static/home/widget/ui/lottery/result.json",
		dataType: "jsonp",
		jsonp: "jsonp",
		jsonpCallback: "ghao123_" + hex_md5(params,16),
		// jsonpCallback: "ghao123_37282f5d74ef27b3",
	    cache: false,
	    success: function(result) {
		    ...
		    if(result && result.message && result.message.errNum >= 0 && result.content && result.content.data && result.content.data.awardInfo && result.content.data.awardInfo.length){
		    	var data = result.content.data;
		    	self._stateCtrl.trigger(self._GET_RESULT_SUCCESS , [data]);
		    }else{
		    	self._stateCtrl.trigger(self._GET_RESULT_FAILURE);
