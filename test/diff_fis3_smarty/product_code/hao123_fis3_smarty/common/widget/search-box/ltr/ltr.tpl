			...
			<%else%>
	            <span>
					<dl id="searchGroupMoreTab" class="box-search_more">
						<dt><a href="#" onClick="return false" hidefocus="true"><%$value.title%></a></dt>
					</dl>
				</span>
			<%/if%>
		<%/foreach%>
	...
	<dd class="box-search_wrap">
		<div class="box-search_inner cf">
			<div class="box-search_logo_wrap">
				...
				<dl class="box-search_logo fl <%if count($sBoxTag[0].engine) - $sBoxCount == 1%>box-search_logo_disabled<%/if%>" id="searchGroupLogos">
					<dt>
			...
			<div class="box-search_form">
				...
				<form id="searchGroupForm" action="<%$value.action%>">
					<span class="o input fl box-search_focus">
			...
			<div class="box-search_hsrch<%if $hSearchWords.hotRightMod == "1"%> box-search_hotright<%/if%>" id="hotSearchWords" log-mod="<%if $hSearchWords.hotRightMod == "1"%>hot-word-right<%else%>hot-word<%/if%>" style="font-size: <%$hSearchWords.fontSize%>px;">
				<%if !empty($hSearchWords.title)%>
					<%if $hSearchWords.hotRightMod == "1"%>
						<div class="hsrch_word hsrch_word_right">
							<%foreach $hSearchWords.words as $val%>
							<%if $val@index < 10%>
						        <span class="hsword-span"><b class="hsword-rank<%if $val@index < 3%> hsword-rank1<%else%> hsword-rank2<%/if%>"><%$val@index+1%></b><a href="<%$val.url|default:'#'%>" onclick="return false" hidefocus="true" data-sort="links" data-val="<%$val.name%>"><%$val.name%></a></span>
						    <%/if%>
						    <%/foreach%>
						</div>
						<div class="hsrch_word_hover">
							<span class="hsrch_title"><%$hSearchWords.title%></span>
						    <div class="hsrch_word">
							    <%foreach $hSearchWords.words as $val%>
								<%if $val@index < 10%>

							        <span class="hsword-span"><b class="hsword-rank<%if $val@index < 3%> hsword-rank1<%else%> hsword-rank2<%/if%>"><%$val@index+1%></b><%if !empty($val.url)%><a href="<%$val.url%>" hidefocus="true" data-sort="links" data-val="<%$val.name%>"><%$val.name%></a><%else%><a href="#" onclick="return false" hidefocus="true" data-sort="links" data-val="<%$val.name%>"><%$val.name%></a><%/if%></span>
							    <%/if%>
							    <%/foreach%>
						   </div>
						</div>
					<%else%>
					    ...
					    <%if empty($hSearchWords.showPersonalHotSearchWords)%>
						    <%foreach $hSearchWords.words as $val%>
						        <%if !empty($val.url)%>
						            <span class="hsword-span"><b class="hsword-rank<%if $val@index < 3%> hsword-rank1<%else%> hsword-rank2<%/if%>"><%$val@index+1%></b><a href="<%$val.url%>" hidefocus="true" data-sort="links" data-val="<%$val.name%>"><%$val.name%></a></span>
						        <%else%>
						            <span class="hsword-span"><b class="hsword-rank<%if $val@index < 3%> hsword-rank1<%else%> hsword-rank2<%/if%>"><%$val@index+1%></b><a href="#" onclick="return false" hidefocus="true" data-sort="links" data-val="<%$val.name%>"><%$val.name%></a></span>
						        <%/if%>
						    <%/foreach%>
						<%/if%>    
					   </span>
