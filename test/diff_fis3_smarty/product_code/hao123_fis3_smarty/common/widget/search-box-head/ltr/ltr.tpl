...
<dl class="box-search<%if $body.searchBox.logoSize == 's'%> box-search-s<%elseif $body.searchBox.logoSize == 'm'%> box-search-m<%/if%> fl" monkey="search" log-mod="search">
					...
					<span class="o input fl box-search_focus">
						<div class="box-search_logo_wrap">
							...
							<dl class="box-search_logo fl <%if count($sBoxTag[0].engine) - $sBoxCount == 1%>box-search_logo_disabled<%/if%>" id="searchGroupLogos">
			                	...
			                	<%$logoIndex = 0%>
			                	<%foreach $sBoxTag[0].engine as $valuedd%>
			                	<%if empty($body.searchboxEngine[$valuedd.id])%>
				                	<dd class="box-search_logo_hide">
				                		<a href="#" onclick="return false" title="<%$valuedd.title%>" hidefocus="true" data-n="<%$logoIndex%>">
						                	<img id="searchGroupLogo_<%$logoIndex%>" src="/resource/fe/img/blank.gif" data-src="<%if !empty($body.searchBox.logoPath)%><%$body.searchBox.logoPath%><%$valuedd.logo%>.png<%else%><%if !empty($head.cdn)%><%$head.cdn%><%/if%>/resource/fe/<%$sysInfo.country%>/search_logo<%if $body.searchBox.logoSize == 's'%>_s<%elseif $body.searchBox.logoSize == 'm'%>_m<%/if%>/<%$valuedd.logo%>.png<%/if%>" alt="<%$valuedd.title%>" />
					                	</a>
				                	</dd> 
				...
				<%if !empty($hSearchWords.title)%>
				    ...
				    <%if empty($hSearchWords.showPersonalHotSearchWords)%>
					    <%foreach $hSearchWords.words as $val%>
					        <%if !empty($val.url)%>
					            <span class="hsword-span"><a href="<%$val.url%>" hidefocus="true" data-sort="links" data-val="<%$val.name%>"><%$val.name%></a></span>
					        <%else%>
					            <span class="hsword-span"><a href="#" onclick="return false" hidefocus="true" data-sort="links" data-val="<%$val.name%>"><%$val.name%></a></span>
					        <%/if%>
					    <%/foreach%>
					<%/if%>    
				   </span>
