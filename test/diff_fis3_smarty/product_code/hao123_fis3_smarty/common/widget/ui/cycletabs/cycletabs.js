...
cycletabs.NavUI.prototype = {
    ...
    init: function(config){
        ...
        // 添加渐显切换支持
        // for需求: hao123首页各种切换动画统一，
        // 升级不影响其他调用该函数的页面
        // by <daihongtao@baidu.com>
        this.animateFade = !!config.animateFade;
    ...
    bindEvent: function(){
        ...
        }).on('mouseenter', '.switch-item', function(){
            var idKey = $(this).attr('data-id');
            for(var i=0; i<that.dataList.length; i++){
                if (that.dataList[i].id == idKey) {
                    break;
    ...
    _scrollNext: function(step){
        ...
        if (this.animateFade) {
            ...
            that.$listContainer.find('[data-index]').first().fadeOut(that._scrollDuration);
            that.$listContainer.find('[data-index]').last().hide().fadeIn(that._scrollDuration, function () {
                $oldItems.remove();
            ...
            //解决在size + step > count 时，动画实现bug的问题
            var removeCount = Math.min(step , that.dataList.length - that.Nav_SIZE);
            ...
            var confEndStyle = {};
            confEndStyle[keyForward] = -that.ITEM_SIZE*removeCount;   //left|right: -xxx px;
    ...
    _scrollPrev: function(step){
        ...
        if (this.animateFade) {
            ...
            var removeCount = Math.min(step, that.dataList.length - that.Nav_SIZE);
            var $oldItems = that.$listContainer.find('[data-index]').slice(-removeCount);  //移除旧的结点

            that.$listContainer.find('[data-index]').last().fadeOut(that._scrollDuration);
            that.$listContainer.find('[data-index]').first().fadeIn(that._scrollDuration, function () {
                $oldItems.remove();
                $(that).trigger('e_after_scroll');
            });
        } else {
            ...
            confStartStyle[keyForward] = -that.ITEM_SIZE*removeCount;   //left: -xxxpx
            confStartStyle[that.DIR_KEY.SIZE] = that.ITEM_SIZE*(that.Nav_SIZE+step);
            var confEndStyle = {};
